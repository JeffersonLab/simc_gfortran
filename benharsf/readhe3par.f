	...
	open(unit=11,file='spec3he_paris.data',status='old')
...
! Convert from fm^-1 to MeV/c, GeV to MeV.  sf is probablility, so unitless.
	...
	open(unit=12,file='benharsf_3par.dat',status='new')
	write(12,*) nump,nume
	do ip=1,nump
	   do ie=1,nume
