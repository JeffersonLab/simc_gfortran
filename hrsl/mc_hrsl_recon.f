...
c	   open (unit=chan,status='old',readonly,file='hrsl/hrs_recon_cosy.dat')
	   open (unit=chan,status='old',file='hrsl/hrs_recon_cosy.dat')
...
! Read in coefficients and exponents.
     ...
     >				expon(specnum,5,n_terms(specnum)))
	   enddo
c	   write(6,*) 'HRSL:N_TERMS, MAX_ORDER = ',n_terms(specnum),max_order
	   close (unit=chan)
	   firsttime = .false.
	endif
