	subroutine mc_hrsl_recon (delta_p,delta_t,delta_phi,y_tgt,fry)

C+______________________________________________________________________________
!
! MC_HRSL_RECON : Reconstruct target quantities from tracks.
!		   This subroutine is part of the MC_HRSL program.
!
...
!		      n_terms variables to specify HRSL. (HRSL = 4)
!
...
C-______________________________________________________________________________
	...
	integer*4 specnum
	parameter (specnum = 4)			!this is the HRSL routine
...
C	integer*2	expon(nspectr,5,max_elements)
	integer*4	n_terms(nspectr),max_order
C	real*8		sum(4),hut(5),term
...
C First time through, read in coefficients from data file.
	...
	if (firsttime) then
	   if (.not.locforunt(chan)) stop 'MC_HRSL_RECON: No I/O channels!'
	   open (unit=chan,status='old',readonly,file='hrsl/hrs_recon_cosy.dat')
...
! Read in coefficients and exponents.
     ...
     >			       (expon(specnum,j,n_terms(specnum)),j=1,4)
	      read (chan,1001) line
...
C     >				expon(specnum,5,n_terms(specnum)))
	   enddo
	   write(6,*) 'HRSL:N_TERMS, MAX_ORDER = ',n_terms(specnum),max_order
	   close (unit=chan)
	   firsttime = .false.
	endif
