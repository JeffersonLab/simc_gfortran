      real*8 function peep_eta_prime(vertex,main)
...
*     photon-NUCLEON center of mass
      ...
      real*8 m_etap/957.78/
      real*8 gamma_etap/0.30/
      real*8 m_omsq, m_p, m_psq
...
C     noted by xucc
      ...
      if (mass**2.lt.m_omsq) then
         write(6,*)mass,sqrt(m_omsq)
         peep_eta_prime=0.0
         return
      endif
...
c      main%t =tt
      ...
      e_etapCM = (Wsq + mass**2 - m_psq)/invm/2.
...
******************************************************************************
**
      sig1 = sig_eta_prime_test(mass/1.e3,qsq/1.e6,invm/1.e3,tt/1.e6,
     *       uu/1.e6,epsilon)
...
CDJG BW in event generation - also want Jacobian at fixed Mx ...
      ...
      peep_eta_prime=Jttheta_fx*gamma_T*ntup%sigcm
...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      real*8 function sig_eta_prime_test(mass,q2_gev,w_gev,tt,uu,epsilon)
...
* all calculations are in GeV
      ...
      sig_etap = a * exp( -b * abs(uu)) + c;

      wfactor=(2.21**2-mp**2)**2/(w_gev**2-mp**2)**2
      sig_etap = sig_etap*wfactor/1.e+06

c     GH: check for weird behavior on upper side of peak
      if (mass.gt.1.5 .and. abs(tt).gt.q2_gev .and. sig_etap.gt.1.e-7)
     *    then
         write(6,*)' tt=',tt,' sig=',sig_etap
         sig_etap=sig_etap*1.e-6
      endif

      sig_eta_prime_test = sig_etap
