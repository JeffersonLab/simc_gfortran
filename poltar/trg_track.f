...
C      write(*,*) mom,mass
      call print_coord2('init track, beam: ',x,y,z,dx,dy)
...
C      write(*,*) ok,spect
      ...
      call print_coord3('init track, z=0:',vt)
...
C      write(*,*) ok,spect
      call print_coord3('init track, z=100:',vt)
...
C that is aesthetics.
      ...
      subroutine transport_proton(main,P_arm_z)
	...
	z_P_arm = -(main.target.x+spec.p.offset.x)*spec.p.sin_th+
     >	           (main.target.z+spec.p.offset.z)*spec.p.cos_th
      ...
      dy_P_arm = main.SP.p.yptar - spec.p.offset.xptar

! project vertex coordinates to z=0 to be used to compare with reconstructed
! target positions

      P_arm_z = y_P_arm - z_P_arm*dy_P_arm

      call track_from_tgt(x_P_arm,y_P_arm,z_P_arm,dx_P_arm,dy_P_arm,
     >                    spec.p.P*(1+delta_P_arm/100.),Mp,+1,ok_P_arm)

      if (.not.ok_P_arm) dx_P_arm = -1000.
...
! construct vectors at first plane of bars for the focal plane coordinates
      ...
      main.FP.p.x  =  x_P_arm + drift_to_ndet*dx_P_arm
      main.FP.p.y  =  y_P_arm + drift_to_ndet*dy_P_arm
      main.FP.p.dx = dx_P_arm
      main.FP.p.dy = dy_P_arm
...
C      write(*,*) 'to target',spect
      call print_coord1('after first mc_hms_recon',y,delta,dx,dy,fry,0.)
...
! and track into the magnetic field to the beam plane (perp. to y)
      ...
      DO WHILE ((delx .GT. .0001) .AND. (n .LT. 10) .AND. ok)
        delx = abs(-fry-vT(1))
	if (debug(6)) write(*,*) '----------------'
	if (debug(6)) write(*,*) 'do while: ',n,delx
        ...
        call print_coord3( 'vT,  beam',vT)
        call print_coord3( 'vTx, beam',vTx)
        CALL trgTrackToPlane (vT, eng,1.,0.,0.,1.,0.,ok,spect)
        CALL trgTrackToPlane (vTx,eng,1.,0.,0.,1.,0.,ok,spect) 
        call print_coord3( 'vT,   z=0',vT)
        call print_coord3( 'vTx,  z=0',vTx)
        ...
        call print_coord1('before mc_hms_recon',y,delta,dx,dy,-xxd,0.)
        ...
        call print_coord1('after mc_hms_recon',y,delta,dx,dy,-xxd,0.)
        ...
        call print_coord3( 'before last track',vT)
        CALL trgTrackToPlane (vT,eng,1.,0.,-ctheta,stheta,frx,ok,spect)
        call print_coord3( 'after last track:',vT)
        n = n+1
      ENDDO
...
! routines to print coordinates given different input - GAW
      ...
      if (debug(6)) write(*,100) txt,vt(1),vt(2),vt(3),vt(4),
 ...
 100  format(a20,6f10.4)
      ...
      if (debug(6)) write(*,100) txt,vt(1),vt(2),vt(3),vt(4)/vt(6),
