...
C-______________________________________________________________________________
	...
	integer*4 specnum
	parameter (specnum = 2)			!this is the SOS routine
...
c	  open (unit=chan,status='old',readonly,file='sos/recon_cosy.dat')
	  open (unit=chan,status='old',file='sos/recon_cosy.dat')
...
! Read in coefficients and exponents.
     ...
     >			expon(specnum,5,n_terms(specnum)))
	  enddo
c	  write(6,*) 'SOS: N_TERMS, MAX_ORDER = ',n_terms(specnum),max_order
	  close (unit=chan)
	  firsttime = .false.
	endif
...
C Actually, make sure they are all non-zero.  The others are one-in-100million
C to be zero (underflow on R*4 from lfit), but it just happened to me).
